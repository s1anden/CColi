...
var Flext = new Class({
    ...
    initialize: function (B, A) {
        ...
        this.autoGrow = B.hasClass(this.options.growClass);
        this.stopEnter = B.hasClass(this.options.enterStoppedClass);
        this.enterSubmits = B.hasClass(this.options.enterSubmitsClass);
        this.useGhostText = B.hasClass(this.options.replaceGhostTextClass);
        this.growParents = B.hasClass(this.options.growParentsClass);
        if (this.autoGrow) {
        ...
        if (this.stopEnter) {
            this.el.addEvent("keydown", function (C) {
                if (C.key == "enter") {
                    C.stop();
    ...
    getMaxSize: function () {
        ...
        if (this.maxSize == 0) {
            var A = this.el.className.match(/maxheight-(\d*)/);
            if (A) {
                this.maxSize = A[1]
            } else {
    ...
    checkSize: function (D) {
        ...
        var B = this.el.getScrollSize();
        if (navigator.userAgent.toLowerCase().indexOf("chrome") > -1) {
            var A = (B.y)
        } else if (navigator.userAgent.match(/AppleWebKit/) && ! navigator.userAgent.match(/Chrome/)) {
			var A = (B.y)
		} else {
            var A = (B.y + this.vertPadding)
        }
        if (A > C.y) {
            this.resizeIt(C, B)
        }
    },
    resizeIt: function (D, C) {
        var A = C.y;

        if ((C.y + this.vertPadding) > this.maxSize && !this.reachedMax) {
            A = this.maxSize;
            this.el.setStyle("overflow", "");
            this.resizer.start("height", A);
            if (this.growParents) {
                var B = A - this.startSize;
                this.resizeParents(this.el, 0, B)
            }
    ...
    resizeParents: function (D, C, B) {
        if (C < this.options.parentDepth) {
            var E = D.getParent();
            if (E) {
                if (E.style.height && E.style.height != "") {
                    if (E.retrieve("flextAdjusted")) {
                        var A = (E.getStyle("height").toInt() + B)
                    } else {
                        E.store("flextAdjusted", true);
                        var A = (E.getStyle("height").toInt() + B + this.vertPadding)
                    }
                    E.setStyle("height", A)
                }
                return this.resizeParents(E, (C + 1), B)
            }
            return true
        } else {
            return true
...
window.addEvent("domready", function () {
    $$("textarea.flext").each(function (B, A) {
        new Flext(B)
    })
});